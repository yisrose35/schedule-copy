...
// daily_adjustments.js  (UPDATED for NEW ENGINE COMPATIBILITY)

// - Correctly calls SchedulerCore.run()

// - Shims getMasterSkeleton to pass daily overrides to the engine

// - Removes dependency on engine return values (engine saves directly)
...
return  <div class="grid-event" data-event-id="${event.id}" title="Click to remove this event" style="${tripStyle || style};padding:2px 5px;border-radius:4px;text-align:center; margin:0 1px;font-size:.9em;position:absolute; top:${top}px;height:${height}px;width:calc(100% - 4px); box-sizing:border-box;overflow:hidden;cursor:pointer;"> ${innerHtml} </div>;
...
// RUN OPTIMIZER — COMPATIBILITY FIX
...
if (!window.SchedulerCore || !SchedulerCore.run) {

alert("Error: SchedulerCore engine not found. Did you load the modules correctly?");
...
// 2. Override window.getMasterSkeleton to return OUR daily blocks

// This allows us to trick the engine (which calls getMasterSkeleton())

// into using the current daily overrides instead of the saved Master Template.

const originalGetMaster = window.getMasterSkeleton;

window.getMasterSkeleton = function() {

return dailyOverrideSkeleton.map(ev => ({
...
subEvents: ev.subEvents ? ev.subEvents.map(s => ({ ...s })) : null
...
// The new engine runs by side-effect (modifying global.scheduleAssignments)

// and handles saving itself. It does not return a result object.
...
window.getMasterSkeleton = originalGetMaster; // Restore original
...
el.innerHTML = <div style="flex-grow:1;"> <div><strong>${item.bunk}</strong> &raquo; ${item.activity}</div> <div class="muted" style="font-size:0.9em;">${item.startTime} - ${item.endTime}</div> </div> <button data-id="${item.id}" style="padding:6px 10px;border-radius:4px;cursor:pointer;background:#c0392b;color:white;border:none;"> Remove </button>;
...
resourceOverridesContainer.innerHTML = <div style="display:flex;flex-wrap:wrap;gap:20px;"> <div style="flex:1;min-width:300px;"> <h4>Fields</h4><div id="override-fields-list" class="master-list"></div> <h4 style="margin-top:15px;">Special Activities</h4><div id="override-specials-list" class="master-list"></div> <h4 style="margin-top:15px;">Leagues</h4><div id="override-leagues-list" class="master-list"></div> <h4 style="margin-top:15px;">Specialty Leagues</h4><div id="override-specialty-leagues-list" class="master-list"></div> </div> <div style="flex:2;min-width:400px;position:sticky;top:20px;"> <h4>Details</h4> <div id="override-detail-pane" class="detail-pane"> <p class="muted">Select an item from the left to edit its details.</p> </div> </div> </div>;
