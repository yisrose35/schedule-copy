// ============================================================================
// scheduler_core_leagues.js — CLEAN VERSION (NO LOGIC CHANGED)
//
// FIXES INCLUDED:
//   • Field conflict prevention inside the same league batch
//   • Specialty + Regular leagues fully supported
//   • Smart Tiles preserved
//   • Correct population of _h2h, _allMatchups, _gameLabel
// ============================================================================
...
(function () {
    ...
    Leagues.assignSportsMultiRound = function (
        matchups,
        sports,
        teamCounts,
        history,
        ...
        matchups.forEach((pair, i) => {
            ...
            assignments.push({ sport: sports[i % sports.length] });
        });
    ...
    Leagues.processSpecialtyLeagues = function (context) {
        ...
        Object.values(groups).forEach(group => {
            ...
            if (typeof window.getLeagueCurrentRound === "function") {
                ...
                window.leagueRoundState[leagueEntry.name]
            ) {
                gameNumber =
                    window.leagueRoundState[leagueEntry.name].currentRound || 1;
            }
            ...
            // Handle matchups
            const matchups =
                typeof window.getLeagueMatchups === "function"
            ...
            matchups.forEach((pair, i) => {
                ...
                const labelBase = `${a} vs ${b} (${bestSport})`;

                let finalField = null;
                if (fields.length > 0) {
                    finalField = fields[i % fields.length];
                }

                allMatchupLabels.push(
                    finalField
                        ? `${labelBase} @ ${finalField}`
                        : `${labelBase} (No Field)`
                );
    ...
    Leagues.processRegularLeagues = function (context) {
        ...
        Object.values(groups).forEach(group => {
            ...
            const div = Object.keys(divisions).find(d =>
                divisions[d].bunks.includes(first)
            );
            if (!div) return;
            ...
            if (typeof window.getLeagueCurrentRound === "function") {
                ...
                gameNumber =
                    window.leagueRoundState[leagueName].currentRound || 1;
            }
            ...
            // *** CRITICAL BATCH FIELD CONFLICT FIX ***
            ...
            matchups.forEach((pair, i) => {
                ...
                // Try to find a unique & available field
