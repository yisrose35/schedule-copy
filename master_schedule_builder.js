// =================================================================
// master_schedule_builder.js (FIXED & AUTO-INIT ENABLED)
//
// Updates:
// 1. Added Auto-Initialization (runs automatically on load).
// 2. Added DOM Polling (waits for container to exist).
// 3. Preserved Grid/Timeline fixes.
// =================================================================
...
(function() {
    ...
    const TILES = [{
        ...
        },
        {
            type: 'dismissal',
            name: 'Dismissal',
            style: 'background:#f44336;color:white;border:1px solid #b71c1c;',
            description: 'Pinned.'
        },
        {
            type: 'custom',
    ...
    function mapEventNameForOptimizer(name) {
        ...
        if (lower === 'activity') return {
            type: 'slot',
            event: 'General Activity Slot'
        };
        if (lower === 'sports') return {
            type: 'slot',
            event: 'Sports Slot'
        };
        if (lower === 'special activity' || lower === 'special') return {
            type: 'slot',
            event: 'Special Activity'
        };
        if (['swim', 'lunch', 'snacks', 'dismissal'].includes(lower)) return {
            type: 'pinned',
            event: name
        };
        return {
    ...
    function init() {
        ...
        // FIX: Retry mechanism. If container doesn't exist yet, wait 100ms and try again.
        if (!container) {
            console.log("Master Scheduler: Container not found yet, retrying...");
            setTimeout(init, 100);
            return;
        }
        ...
        if (savedDraft) {
            // We use a simple confirm here. In production, a custom modal is better.
            // Using a try-catch to prevent alert spam loop if something is weird.
            try {
                ...
                console.warn("Auto-load skipped due to interaction limitations.");
            }
        }
        ...
        ui.innerHTML = `
            <div class="template-toolbar" style="display:flex;flex-wrap:wrap;gap:20px;align-items:flex-end;">
              ...
              <div class="template-group">
                <label> </label>
                <button id="template-save-btn" style="background:#007bff;color:#fff;padding:6px 12px;border:none;border-radius:4px;cursor:pointer;">Save</button>
                <button id="template-clear-btn" style="background:#ff9800;color:#fff;margin-left:8px;padding:6px 12px;border:none;border-radius:4px;cursor:pointer;">New</button>
              </div>
            </div>
            <details style="margin-top:10px;">
              ...
              <div style="margin-top:10px;padding:10px;border:1px solid #eee;">
                ...
                <button id="template-assign-save-btn" style="margin-top:10px;background:#28a745;color:white;padding:6px 12px;border:none;border-radius:4px;cursor:pointer;">Save Assignments</button>
                <hr>
                <button id="template-delete-btn" style="background:#c0392b;color:white;padding:6px 12px;border:none;border-radius:4px;cursor:pointer;">Delete Selected Template</button>
              </div>
            </details>
        `;
        // Bindings
    ...
    function renderGrid() {
        ...
        if (availableDivisions.length === 0) {
            // FIX: If data is missing, provide a clear message rather than a blank div
            grid.innerHTML = `<div style="padding:20px;text-align:center;color:#666;border:1px dashed #ccc;margin:20px;">
                <strong>No divisions found.</strong><br>
                Please initialize window.divisions and window.availableDivisions, or go to Setup.
            </div>`;
            return;
        }
    ...
    function renderEventTile(ev, top, height) {
        ...
        return `<div class="grid-event" data-id="${ev.id}" title="Click to remove" style="${style}; position:absolute; top:${top}px; height:${height}px; width:96%; left:2%; padding:2px; font-size:0.85rem; overflow:hidden; border-radius:4px; cursor:pointer;">
            ${label}
        </div>`;
    }
        ...
        grid.querySelectorAll(selector).forEach(cell => {
            ...
            cell.ondrop = e => {
                ...
                // 2. Split Tile
                    ...
                    let et = prompt("End Time:", endStr);
                    if (!et) return;
                    let a1 = prompt("Activity 1 (First Half):");
                    if (!a1) return;
                    let a2 = prompt("Activity 2 (Second Half):");
                    if (!a2) return;
                    newEvent = {
                        id: Date.now().toString(),
                        type: 'split',
    ...
    // 1. Expose to window so other scripts can call it if needed
    window.initMasterScheduler = init;

    // 2. AUTO-INIT with DOM Ready Check
    // This is the key fix. It attempts to run immediately, or on load.
