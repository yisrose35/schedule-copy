/* ===========================================================================
   ...
   Fixed: Removed extra closing brace at end of file.
   ===========================================================================*/
...
/* ---------------------------------------------------------------------------
   HELPER: Assign & Mark
--------------------------------------------------------------------------- */
function assign(bunk, block, pick, ctx) {
    if (!pick) return;
...
function findBestSport(block, allActivities, fieldUsage, avoidField, activityProps) {
    ...
    const fieldsBySport = allActivities.fieldsBySport || {};
...
function findBestGeneral(block, allActivities, fieldUsage, avoidField, activityProps) {
    // Try specials first (often preferred for general slots)
    const specialPick = findBestSpecial(block, allActivities, fieldUsage, avoidField, activityProps);
...
/* ---------------------------------------------------------------------------
   MAIN RUN: Process Standard Slots
--------------------------------------------------------------------------- */
function run(blocks, ctx) {
    // 1. Identify "Slot" blocks (not smart, league, specialty, or pinned)
    const slotBlocks = blocks.filter(b => 
        b.type === 'slot' || 
        (b.type !== 'smart' && b.type !== 'league' && b.type !== 'specialty' && b.type !== 'pinned' && b.type !== 'split')
    );
