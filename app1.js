...
(function () {
  ...
  function ensureSharedSetupStyles() {
    ...
    style.textContent = `
        ...
        .sport-cap-input {
            width: 60px; padding: 4px; border: 1px solid #D1D5DB;
  ...
  function handleBulkImport(file) {
    ...
    reader.onload = e => {
      ...
      let addedDivs = 0; let addedBunks = 0; let updatedSizes = 0;
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        if (divName) {
            ...
            if (!availableDivisions.includes(divName)) {
                ...
                divisions[divName] = { bunks: [], color: defaultColors[colorIndex++ % defaultColors.length] };
                addedDivs++;
            }
            // Add Bunk
            if (bunkName) {
  ...
  function showSportsRulesModal() {
    ...
    modal.innerHTML = `
      ...
      <div style="margin-top:22px; text-align:right;">
        <button id="save-sports-rules" style="background:#00C896; color:white; border:none; padding:8px 20px; border-radius:999px; cursor:pointer; font-weight:600;">Save Rules</button>
      </div>
    `;
    ...
    sortedSports.forEach(sport => {
      ...
      row.innerHTML = `<strong>${sport}</strong><div><span style="font-size:0.8rem; margin-right:5px;">Max Kids:</span><input type="number" class="sport-cap-input" data-sport="${sport}" value="${cap}" placeholder="∞"></div>`;
  ...
  function renderDivisionDetailPane() {
    ...
    deleteBtn.style.cssText = "background:#FFFFFF; color:#DC2626; border:1px solid #FECACA; padding:6px 16px; border-radius:999px; cursor:pointer; font-weight:600; font-size:0.85rem; box-shadow:0 4px 10px rgba(220,38,38,0.12);";
    deleteBtn.onclick = () => {
    ...
    makeEditable(titleName, (newName) => {
      ...
      if (divisions[trimmed]) { alert("Exists."); return; }
    ...
    const timesSummary = divObj.startTime && divObj.endTime ? `${divObj.startTime} \u2013 ${divObj.endTime}` : "Times not set";
    
    // Updated Summary with Total Campers
    rightSide.innerHTML = `${bunkCount} bunks \u2022 <strong>${totalKids}</strong> campers \u2022 ${timesSummary}`;
    ...
    timeCard.innerHTML = `<div class="division-mini-header"><span>Division Times</span></div><p class="division-mini-help">Set the daily time window this division is in camp.</p>`;
    ...
    startInput.style.cssText = "width:80px; padding:4px 8px; border-radius:999px; border:1px solid #D1D5DB; font-size:0.85rem;";
    
    const toLabel = document.createElement("span");
    toLabel.textContent = "to"; toLabel.className = "muted";
    
    const endInput = document.createElement("input");
    endInput.value = divObj.endTime || "";
    endInput.placeholder = "4:00pm";
    endInput.style.cssText = "width:80px; padding:4px 8px; border-radius:999px; border:1px solid #D1D5DB; font-size:0.85rem;";

    const updateBtn = document.createElement("button");
    updateBtn.textContent = "Save";
    updateBtn.style.cssText = "background:#00C896; color:white; border:none; padding:4px 12px; border-radius:999px; font-weight:600; cursor:pointer;";
    updateBtn.onclick = () => {
    ...
    addInput.style.cssText = "flex:1; padding:5px 10px; border-radius:999px; border:1px solid #D1D5DB; font-size:0.86rem;";
    const addBtn = document.createElement("button");
    addBtn.textContent = "Add";
    addBtn.style.cssText = "padding:5px 14px; border-radius:999px; border:none; background:#00C896; color:white; font-size:0.85rem; font-weight:600; cursor:pointer;";
