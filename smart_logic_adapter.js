...
Solver.solveSchedule = function (allBlocks, config) {
    ...
    const solvedLeague = Solver.solveLeagues(buckets.special_league, true);
    Solver.solveLeagues(buckets.league, false);

    // 2. SMART TILES
    Solver.solveSmartTiles(buckets.smart);

    // 3. SPLIT TILES
    Solver.solveSplits(buckets.split);

    // 4. GENERAL BACKTRACK SOLVER
    ...
    // Need to initialize buckets since they are used in phase logic but not defined in the provided snippet's solveSchedule
    // The provided snippet had buckets defined inside solveSchedule but the user provided a snippet where solveSchedule 
    // uses sortBlocksByDifficulty and then filters.
    // Wait, the provided snippet has a logic gap: it calls Solver.solveLeagues(buckets...) but buckets isn't defined in the new solveSchedule function provided in the text.
    // I need to reconstruct the buckets logic to match the new architecture features listed in the comments.
    
    // Correcting the solveSchedule to use the phase logic correctly based on the provided text's intention.
    
    // 1. Group Blocks into Phases
    const buckets = {
        special_league: [],
        league: [],
        smart: [],
    ...
    allBlocks.forEach(b => {
        if (b._isSmart) buckets.smart.push(b);
        else if (b.type === 'split') buckets.split.push(b);
        else if (b.type === 'specialty_league') buckets.special_league.push(b);
        else if (b._isLeague) buckets.league.push(b);
        else buckets.general.push(b);
    });

    // 2. Strict Execution Order
    
    // 1. Special Leagues
    Solver.solveLeagues(buckets.special_league, true);
    ...
    // 5. General Activity (Backtracking)
    // The provided text uses backtrack() on activityBlocks. 
    // activityBlocks basically corresponds to buckets.general in this new structure.
    
    const generalBlocks = buckets.general;
    
    iterations = 0; // Reset
    
    function backtrackGeneral(idx) {
        iterations++;
        if (iterations > MAX_ITERATIONS) return true;
        if (idx === generalBlocks.length) return true;

        const block = generalBlocks[idx];
        ...
        for (const p of picks) {
            const res = Solver.applyTentativePick(block, p);
            if (backtrackGeneral(idx + 1)) return true;
            Solver.undoTentativePick(res);
        }
