...
//
// UPDATED:
// - Correctly calculates "Game Number" using the global absolute counter.
// - Supports multiple league blocks in one day incrementing correctly (Game 1, Game 2...).
// - Removes modulo logic for labels so games can go beyond the number of teams (Game 6+).
// ============================================================================
...
(function() {
    ...
    window.runSkeletonOptimizer = function(manualSkeleton, externalOverrides) {
        ...
        function markFieldUsage(block, fieldName, fieldUsageBySlot) {
            if (!fieldName || fieldName === "No Field" || !window.allSchedulableNames || !window.allSchedulableNames.includes(fieldName)) return;
        ...
        function isTimeAvailable(slotIndex, fieldProps) {
            ...
            let isAvailable = !rules.some(r => r.type === 'Available');
        ...
        function canBlockFit(block, fieldName, activityProperties, fieldUsageBySlot, proposedActivity) {
            ...
            if (props.allowedDivisions?.length > 0 && !props.allowedDivisions.includes(block.divName)) return false;
            ...
            if (rules.length > 0) {
                ...
                if (blockStartMin != null && blockEndMin != null) {
                    if (hasAvailableRules) {
                        ...
                        for (const rule of rules) {
                            if (rule.type === 'Available' && rule.startMin != null && rule.endMin != null && blockStartMin >= rule.startMin && blockEndMin <= rule.endMin) {
                                insideAvailable = true; break;
                            }
                        }
                        if (!insideAvailable) return false;
                    }
                    for (const rule of rules) {
                        if (rule.type === 'Unavailable' && rule.startMin != null && rule.endMin != null && blockStartMin < rule.endMin && blockEndMin > rule.startMin) return false;
                    }
                }
                for (const slotIndex of block.slots || []) {
            ...
            (block.slots || []).forEach((slotIndex, idx) => {
                ...
                if (!window.scheduleAssignments[block.bunk][slotIndex]) {
                    window.scheduleAssignments[block.bunk][slotIndex] = {
                        ...
                        _gameLabel: pick._gameLabel || null // <-- SAVE GAME NUMBER
                    };
                    if (!isLeagueFill && fieldName && window.allSchedulableNames && window.allSchedulableNames.includes(fieldName)) {
                        fieldUsageBySlot[slotIndex] = fieldUsageBySlot[slotIndex] || {};
                        const usage = fieldUsageBySlot[slotIndex][fieldName] || { count: 0, divisions: [], bunks: {} };
                        usage.count++;
                        if (!usage.divisions.includes(block.divName)) usage.divisions.push(block.divName);
        ...
        function loadAndFilterData() {
            ...
            const availableDivisions = (app1Data.availableDivisions || []).filter(divName => !overrides.bunks.includes(divName));
            ...
            allMasterActivities.forEach(f => {
                ...
                if (Array.isArray(f.allowedDivisions) && f.allowedDivisions.length > 0) allowedDivisions = f.allowedDivisions.slice();
                else if (f.divisionAvailability?.mode === 'specific') allowedDivisions = f.divisionAvailability.divisions.slice();
                else if (f.sharableWith?.divisions?.length > 0) allowedDivisions = f.sharableWith.divisions.slice();

                const safeLimitUsage = (f.limitUsage && f.limitUsage.enabled) ? { enabled: true, divisions: f.limitUsage.divisions || {} } : { enabled: false, divisions: {} };
            ...
            return {
                divisions, availableDivisions, activityProperties, allActivities, h2hActivities, fieldsBySport, masterLeagues, masterSpecialtyLeagues, masterSpecials,
                yesterdayHistory, rotationHistory, disabledLeagues, disabledSpecialtyLeagues, historicalCounts, specialActivityNames, disabledFields, disabledSpecials,
                dailyFieldAvailability, dailyDisabledSportsByField, masterFields 
            };
        }
        ...
        for (let i = 0; i < sortedPoints.length - 1; i++) {
            ...
            if (end - start >= 5) {
                window.unifiedTimes.push({ start: minutesToDate(start), end: minutesToDate(end), label: `${fmtTime(minutesToDate(start))} - ${fmtTime(minutesToDate(end))}` });
            }
        }
            ...
            if ((item.type === 'pinned' || !isGeneratedEvent) && item.type !== 'smart') {
                ...
                allBunks.forEach(bunk => {
                    allSlots.forEach((slotIndex, idx) => {
                        if (!window.scheduleAssignments[bunk][slotIndex]) {
                            window.scheduleAssignments[bunk][slotIndex] = {
                                field: { name: item.event }, sport: null, continuation: (idx > 0), _fixed: true, _h2h: false, vs: null, _activity: item.event, _endTime: endMin
                            };
                ...
                function pinSwim(bunks, slots) {
                    bunks.forEach(bunk => {
                        slots.forEach((slotIndex, idx) => {
                            window.scheduleAssignments[bunk][slotIndex] = {
                                field: { name: swimLabel }, sport: null, continuation: (idx > 0), _fixed: true, _h2h: false, vs: null, _activity: swimLabel
                            };
                ...
                function pushGA(bunks, slots) {
                    bunks.forEach(bunk => {
                        schedulableSlotBlocks.push({
                            divName: item.division, bunk: bunk, event: gaLabel, startTime: startMin, endTime: endMin, slots
                        });
                    });
                }
                pinSwim(bunksTop, slotsFirst); pushGA(bunksBottom, slotsFirst); pushGA(bunksTop, slotsSecond); pinSwim(bunksBottom, slotsSecond);
                ...
                allBunks.forEach(bunk => {
                    schedulableSlotBlocks.push({
                        divName: item.division, bunk: bunk, event: normalizedEvent, startTime: startMin, endTime: endMin, slots: allSlots
                    });
                });
            }
        });
        ...
        smartJobs.forEach(job => {
            ...
            const adapterResult = SmartLogicAdapter.generateAssignments(
                bunks, job, historicalCounts, specialActivityNames, activityProperties, masterFields, dailyFieldAvailability, yesterdayHistory
            );
            ...
            function pushGenerated(bunk, event, startMin, endMin) {
                const slots = findSlotsForRange(startMin, endMin);
                schedulableSlotBlocks.push({
                    divName: job.division, bunk, event, startTime: startMin, endTime: endMin, slots, fromSmartTile: true
                });
            }
            const slotsA = findSlotsForRange(job.blockA.startMin, job.blockA.endMin);
            Object.entries(block1Assignments).forEach(([bunk, act]) => {
                if (act.toLowerCase().includes("sport")) { pushGenerated(bunk, "Sports Slot", job.blockA.startMin, job.blockA.endMin); return; }
                if (act.toLowerCase().includes("special")) { pushGenerated(bunk, "Special Activity Slot", job.blockA.startMin, job.blockA.endMin); return; }
                if (act.toLowerCase().includes("general activity")) { pushGenerated(bunk, "General Activity Slot", job.blockA.startMin, job.blockA.endMin); return; }
                slotsA.forEach((slotIndex, idx) => {
                    if (!window.scheduleAssignments[bunk]) window.scheduleAssignments[bunk] = [];
                    if (!window.scheduleAssignments[bunk][slotIndex]) {
                        window.scheduleAssignments[bunk][slotIndex] = {
                            field: { name: act }, sport: null, continuation: (idx > 0), _fixed: true, _h2h: false, vs: null, _activity: act, _endTime: job.blockA.endMin
                        };
            ...
            if (job.blockB) {
                ...
                Object.entries(block2Assignments).forEach(([bunk, act]) => {
                    if (act.toLowerCase().includes("sport")) { pushGenerated(bunk, "Sports Slot", job.blockB.startMin, job.blockB.endMin); return; }
                    if (act.toLowerCase().includes("special")) { pushGenerated(bunk, "Special Activity Slot", job.blockB.startMin, job.blockB.endMin); return; }
                    if (act.toLowerCase().includes("general activity")) { pushGenerated(bunk, "General Activity Slot", job.blockB.startMin, job.blockB.endMin); return; }
                    slotsB.forEach((slotIndex, idx) => {
                        if (!window.scheduleAssignments[bunk]) window.scheduleAssignments[bunk] = [];
                        if (!window.scheduleAssignments[bunk][slotIndex]) {
                            window.scheduleAssignments[bunk][slotIndex] = {
                                field: { name: act }, sport: null, continuation: (idx > 0), _fixed: true, _h2h: false, vs: null, _activity: act, _endTime: job.blockB.endMin
                            };
                            registerSingleSlotUsage(slotIndex, act, job.division, bunk, act);
                        }
                    });
        ...
        const remainingBlocks = schedulableSlotBlocks.filter(b => b.event !== 'League Game' && b.event !== 'Specialty League' && !b.processed);
        ...
        specialtyLeagueBlocks.forEach(block => {
            ...
            if (!specialtyLeagueGroups[key]) {
                specialtyLeagueGroups[key] = { divName: block.divName, startTime: block.startTime, endTime: block.endTime, slots: block.slots, bunks: new Set() };
            }
        ...
        Object.values(specialtyLeagueGroups).forEach(group => {
            ...
            if (typeof window.getLeagueCurrentRound === 'function') {
                 ...
                 // Fallback if league_scheduling.js isn't updated
        ...
        leagueBlocks.forEach(block => {
            ...
            if (!leagueGroups[key]) {
                leagueGroups[key] = { leagueName, league: leagueEntry[1], startTime: block.startTime, endTime: block.endTime, slots: block.slots, bunks: new Set() };
            }
        ...
        sortedLeagueGroups.forEach(group => {
            ...
            if (bestResult.successCount < nonByeCount) {
                ...
                for (let i = 0; i < 50; i++) {
                    ...
                    if (res.successCount > bestResult.successCount) { bestResult = res; if (res.successCount === nonByeCount) break; }
                }
            }
            ...
            usedForAssignments.forEach(game => {
                ...
                bunkPtr += 2;
                const pick = { field: game.field, sport: game.label, _h2h: true, vs: null, _activity: game.sport, _allMatchups: allMatchupLabels, _gameLabel: gameLabel };
        ...
        for (const block of remainingBlocks) {
            ...
            if (pick) {
                ...
                if (pick._activity && block.bunk) {
                    ...
                    if (isSpecial) { historicalCounts[block.bunk]['_totalSpecials'] = (historicalCounts[block.bunk]['_totalSpecials'] || 0) + 1; }
                }
            } else {
                    ...
                    for (const entry of schedule) {
                        if (entry && entry._activity && entry._activity !== lastActivity) {
                            ...
                            if (entry._h2h && activityName !== "League" && activityName !== "No Game") {
                                const leagueEntry = Object.entries(masterLeagues).find(([name, l]) => l.enabled && l.divisions && l.divisions.includes(divName));
                                if (leagueEntry) {
