...
//
// UPDATED (Usage Limit Logic):
// - Calculates 'historicalCounts' by scanning all past daily schedules.
// - Merges in 'manualUsageOffsets' from Global Settings.
// - Passes 'historicalCounts' to filler functions.
// ============================================================================
...
// =====================================================================
// CALCULATE HISTORICAL USAGE (Across all saved days)
// =====================================================================
function calculateEffectiveHistoricalCounts() {
    const allDailyData = window.loadAllDailyData?.() || {};
    const globalSettings = window.loadGlobalSettings?.() || {};
    const manualOffsets = globalSettings.manualUsageOffsets || {};
    
    const historicalCounts = {}; // { bunk: { activity: count } }

    for (const dayKey in allDailyData) {
        // Don't count today if we are actively regenerating it
        if (dayKey === window.currentScheduleDate) continue; 

        const daySched = allDailyData[dayKey].scheduleAssignments || {};
        for (const bunk in daySched) {
            const sched = daySched[bunk] || [];
            for (const entry of sched) {
                // Count every non-continuation block that is a valid activity
                if (entry && entry._activity && !entry.continuation && !entry._h2h && entry._activity !== "Free" && entry._activity !== "No Field") {
                    if (!historicalCounts[bunk]) historicalCounts[bunk] = {};
                    historicalCounts[bunk][entry._activity] = (historicalCounts[bunk][entry._activity] || 0) + 1;
                }
            }
        }
    }
    ...
    for (const bunk in manualOffsets) {
        for (const act in manualOffsets[bunk]) {
            if (!historicalCounts[bunk]) historicalCounts[bunk] = {};
            const current = historicalCounts[bunk][act] || 0;
            // Effective can't be less than 0
            historicalCounts[bunk][act] = Math.max(0, current + manualOffsets[bunk][act]);
        }
    }
...
window.runSkeletonOptimizer = function(manualSkeleton) {
    ...
    // --- NEW: Calculate Historical Usage for Limits ---
    const historicalCounts = calculateEffectiveHistoricalCounts();
    ...
    for (const block of remainingBlocks) {
        ...
        // 1) Specific buckets
            ...
            pick = window.findBestSpecial?.(
                ...
                rotationHistory,
                divisions,
                historicalCounts // --- UPDATED: Passed historical counts
            );
...
function loadAndFilterData() {
    ...
    allMasterActivities.forEach(f => {
        ...
        activityProperties[f.name] = {
            ...
            maxUsage: f.maxUsage || 0 // --- NEW: Load Max Usage
        };
