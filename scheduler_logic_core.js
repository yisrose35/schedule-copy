// ============================================================================// scheduler_logic_core.js//// SUPER GEMINI FIX:// - FIXED: 'processLeagues' was defined but NEVER CALLED. Added execution steps.// - FIXED: 'findSlotsForRange' used strict containment, causing blocks like//          12:20-1:20 to miss the 12:00 slot or misalign. Now uses OVERLAP logic.// - SAFETY: Added checks to prevent crashes if 'allActivities' is empty.// ============================================================================(function () {'use strict';// ===== CONFIG =====const INCREMENT_MINS = 30;window.INCREMENT_MINS = INCREMENT_MINS;const GENERATED_EVENTS = ['General Activity Slot','Sports Slot','Special Activity','Swim','League Game','Specialty League'];// =============================================================// 1. GLOBAL AVAILABILITY MANAGER// =============================================================const GlobalAvailabilityManager = (function() {let reservations = {};function reset() { reservations = {}; }
...
})();window.GlobalAvailabilityManager = GlobalAvailabilityManager;// ===== BASIC HELPERS =====function parseTimeToMinutes(str) {if (str == null) return null;if (typeof str === 'number') return str;let s = str.trim().toLowerCase().replace(/am|pm/g, (m) => (m === 'am' ? 'am' : 'pm')).replace(/\s+/g, '');let mer = s.includes('pm') ? 'pm' : s.includes('am') ? 'am' : null;s = s.replace(/am|pm/, '');const m = s.match(/^(\d{1,2}):?(\d{2})?$/);if (!m) return null;let hh = parseInt(m[1], 10);const mm = m[2] ? parseInt(m[2], 10) : 0;if (mer) {if (hh === 12) hh = mer === 'am' ? 0 : 12;else if (mer === 'pm') hh += 12;}return hh * 60 + mm;}function fieldLabel(f) { return (f && typeof f === 'object' && f.name) ? f.name : f; }function fmtTime(d) {if (!d) return '';if (typeof d === 'string') d = new Date(d);let h = d.getHours();let m = d.getMinutes().toString().padStart(2, '0');const ap = h >= 12 ? 'PM' : 'AM';h = h % 12 || 12;return `${h}:${m} ${ap}`;}function shuffleArray(array) {for (let i = array.length - 1; i > 0; i--) {const j = Math.floor(Math.random() * (i + 1));[array[i], array[j]] = [array[j], array[i]];}return array;}// ======================================================// LEAGUE ROUND STATE// ======================================================let coreLeagueRoundState = window.coreLeagueRoundState || {};(function initCoreLeagueRoundState() {try {const daily = window.loadCurrentDailyData?.() || {};coreLeagueRoundState = daily.coreLeagueRoundState || {};} catch (e) { coreLeagueRoundState = {}; }window.coreLeagueRoundState = coreLeagueRoundState;})();function saveCoreLeagueRoundState() {window.saveCurrentDailyData?.('coreLeagueRoundState', coreLeagueRoundState);}function coreFullRoundRobin(teamList) {if (!teamList || teamList.length < 2) return [];const teams = teamList.map(String);const t = [...teams];if (t.length % 2 !== 0) t.push('BYE');const n = t.length;const fixed = t[0];let rotating = t.slice(1);const rounds = [];for (let r = 0; r < n - 1; r++) {const pairings = [];pairings.push([fixed, rotating[0]]);for (let i = 1; i < n / 2; i++) {pairings.push([rotating[i], rotating[rotating.length - i]]);}rounds.push(pairings.filter(([a, b]) => a !== 'BYE' && b !== 'BYE'));rotating.unshift(rotating.pop());}return rounds;}function coreGetNextLeagueRound(leagueName, teams) {const key = String(leagueName || '');if (!key || !teams || teams.length < 2) return [];const teamKey = teams.map(String).sort().join('|');const rounds = coreFullRoundRobin(teams);if (rounds.length === 0) return [];let state = coreLeagueRoundState[key] || { idx: 0, teamKey };
...
return matchups;
}function pairRoundRobin(teamList) {const arr = teamList.map(String);if (arr.length < 2) return [];if (arr.length % 2 === 1) arr.push('BYE');const n = arr.length;const pairs = [];for (let i = 0; i < n / 2; i++) {const A = arr[i];const B = arr[n - 1 - i];if (A !== 'BYE' && B !== 'BYE') pairs.push([A, B]);}return pairs;}// ====== LEAGUE SPORT OPTIMIZER ======function assignSportsMultiRound(matchups, availableLeagueSports, existingTeamCounts, leagueHistory, lastSportByTeamBase) {const sports = availableLeagueSports.slice();const baseTeamCounts = existingTeamCounts || {};const allTeams = new Set();matchups.forEach(([a, b]) => { if(a && b) { allTeams.add(String(a)); allTeams.add(String(b)); }});const workCounts = {};
...
}// =====================================================================// MAIN ENTRY POINT// =====================================================================window.runSkeletonOptimizer = function (manualSkeleton) {window.scheduleAssignments = {};window.leagueAssignments = {};window.unifiedTimes = [];window.GlobalAvailabilityManager.reset();
...
manualSkeleton.forEach((item) => {
    ...
    if (item.type === 'pinned' || !isGenerated) {
        allBunks.forEach(bunk => {
         ...
         const pin = (bunks, slots, act, sTime, eTime) => {
             bunks.forEach(b => {
                 slots.forEach((idx, i) => {
                 ...
                 markFieldUsage({divName:item.division, bunk:b, startTime:sTime, endTime:eTime, slots}, act, fieldUsageBySlot);
             });
         };
         const push = (bunks, slots, act, sTime, eTime) => {
             bunks.forEach(b => schedulableSlotBlocks.push({
                 divName: item.division, bunk:b, event:act, startTime:sTime, endTime:eTime, slots
             }));
         };
         ...
         // 1st Half (Start -> Mid)
         pin(bunksTop, slots1, 'Swim', startMin, midTime);
         push(bunksBot, slots1, gaName, startMin, midTime);
         
         // 2nd Half (Mid -> End)
         push(bunksTop, slots2, gaName, midTime, endMin);
         pin(bunksBot, slots2, 'Swim', midTime, endMin);
...
// =================================================================// PASS 3 & 3.5: LEAGUES// =================================================================const processLeagues = (blocks, isSpecialty) => {const groups = {};blocks.forEach(b => {const k = `${b.divName}-${b.startTime}`;if (!groups[k]) groups[k] = { ...b, bunks: new Set() };groups[k].bunks.add(b.bunk);});Object.values(groups)
...
};// --- EXECUTE LEAGUE PROCESSORS (MISSING IN PREVIOUS VERSION) ---const specialtyBlocks = schedulableSlotBlocks.filter(b => b.event === 'Specialty League');if (specialtyBlocks.length > 0) processLeagues(specialtyBlocks, true);const leagueBlocks = schedulableSlotBlocks.filter(b => b.event === 'League Game');if (leagueBlocks.length > 0) processLeagues(leagueBlocks, false);// =================================================================
// PASS 4: FILLERS
// =================================================================
...
return true;
};// =====================================================================// HELPERS// =====================================================================// FIX: Detects ANY overlap between the block and the slotfunction findSlotsForRange(startMin, endMin) {const slots = [];if (!window.unifiedTimes || startMin == null || endMin == null) return slots;window.unifiedTimes.forEach((slot, i) => {const slotStart = slot.start.getHours()*60 + slot.start.getMinutes();const slotEnd = slotStart + INCREMENT_MINS;// Overlap Logic: Max(startA, startB) < Min(endA, endB)if (Math.max(startMin, slotStart) < Math.min(endMin, slotEnd)) {slots.push(i);}});return slots;}function getBlockTimeRange(block) {let blockStartMin = typeof block.startTime === 'number' ? block.startTime : null;let blockEndMin = typeof block.endTime === 'number' ? block.endTime : null;if (window.unifiedTimes && Array.isArray(block.slots) && block.slots.length > 0) {
...
}function markFieldUsage(block, fieldName, fieldUsageBySlotLocal) {if (!fieldName || fieldName === 'No Field' || !window.allSchedulableNames?.includes(fieldName)) return;const { blockStartMin, blockEndMin } = getBlockTimeRange(block);
...
}function fillBlock(block, pick, fieldUsageBySlotLocal, yesterdayHistory, isLeagueFill) {const fieldName = fieldLabel(pick.field);block.slots.forEach((idx, i) => {if (!window.scheduleAssignments[block.bunk][idx]) {window.scheduleAssignments[block.bunk][idx] = {field: fieldName,sport: pick.sport,continuation: i > 0,_fixed: !!pick._fixed,_h2h: pick._h2h || false,vs: pick.vs || null,_activity: pick._activity || null,_allMatchups: pick._allMatchups || null};}});if (!isLeagueFill && fieldName) {markFieldUsage({ ...block, _activity: pick._activity }, fieldName, fieldUsageBySlotLocal);}}function isTimeAvailable(timeObj, fieldProps) {let s, e;if (typeof timeObj === 'number') {if (!window.unifiedTimes || !window.unifiedTimes[timeObj]) return false;const slot = window.unifiedTimes[timeObj];s = new Date(slot.start).getHours() * 60 + new Date(slot.start).getMinutes();e = s + INCREMENT_MINS;} else {s = timeObj.startTime;e = timeObj.endTime;}const rules = (fieldProps.timeRules || []).map((r) => {
...
return isAvailable;
}// =====================================================================// DATA LOADER// =====================================================================function loadAndFilterData() {const globalSettings = window.loadGlobalSettings?.() || {};const app1Data = globalSettings.app1 || {};const masterFields = app1Data.fields || [];const masterDivisions = app1Data.divisions || {};const masterAvailableDivs = app1Data.availableDivisions || [];const masterSpecials = app1Data.specialActivities || [];const masterLeagues = globalSettings.leaguesByName || {};const masterSpecialtyLeagues = globalSettings.specialtyLeagues || {};const dailyData = window.loadCurrentDailyData?.() || {};const dailyFieldAvailability = dailyData.dailyFieldAvailability || {};const dailyOverrides = dailyData.overrides || {};const disabledLeagues = dailyOverrides.leagues || [];const disabledSpecialtyLeagues = dailyData.disabledSpecialtyLeagues || [];const dailyDisabledSportsByField = dailyData.dailyDisabledSportsByField || {};const disabledFields = dailyOverrides.disabledFields || [];const disabledSpecials = dailyOverrides.disabledSpecials || [];const rotationHistoryRaw = window.loadRotationHistory?.() || {};const rotationHistory = { bunks: rotationHistoryRaw.bunks || {}, leagues: rotationHistoryRaw.leagues || {}, leagueTeamSports: rotationHistoryRaw.leagueTeamSports || {}, leagueTeamLastSport: rotationHistoryRaw.leagueTeamLastSport || {} };const historicalCounts = {};
try {
